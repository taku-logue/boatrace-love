...
from .layouts import RACE_RESULT_LAYOUT_V1 # 上部でimportされているか確認

def parse_race_result_line(line: str) -> dict[str, str]:
    # （※この関数は変更なしなのでそのまま残しておいてください）
    parsed = {}
    for key, (start, length) in RACE_RESULT_LAYOUT_V1.items():
        if len(line) >= start:
            parsed[key] = line[start:start+length].strip()
...
def parse_race_result_file(filepath: str, download_file_id: int, raw_file_id: int, race_date: date):
    """競走成績ファイル全体を読み込み、DB保存用のレコードリストを生成する"""
    raw_records, result_records, payout_records = [], [], [] # payout_recordsを追加
    race_records_dict = {}
    ...
    with open(filepath, "r", encoding="cp932", errors="replace") as f:
        for line_num, line in enumerate(f, start=1):
            ...
            summary_match = re.search(r"^\s*([0-9]{1,2})R\s+([1-6]-[1-6]-[1-6])\s+(\d+)\s+([1-6]-[1-6]-[1-6])\s+(\d+)\s+([1-6]-[1-6])\s+(\d+)\s+([1-6]-[1-6])\s+(\d+)", raw_text)
            if summary_match and current_venue:
                ...
                payout_records.append({"race_id": race_id, "bet_type": "trifecta", "combination": summary_match.group(2), "payout_yen": int(summary_match.group(3))})
                payout_records.append({"race_id": race_id, "bet_type": "trio", "combination": summary_match.group(4), "payout_yen": int(summary_match.group(5))})
                payout_records.append({"race_id": race_id, "bet_type": "exacta", "combination": summary_match.group(6), "payout_yen": int(summary_match.group(7))})
                payout_records.append({"race_id": race_id, "bet_type": "quinella", "combination": summary_match.group(8), "payout_yen": int(summary_match.group(9))})
                continue
            ...
            if race_match and current_venue:
                ...
                current_decision = None # レースが変わるごとに決まり手をリセット
