...
def extract_lzh(lzh_path: str, output_dir: str) -> str | None:
    ...
    try:
        ...
        result = subprocess.run(
            ["7z", "e", lzh_path, f"-o{output_dir}", "-y"], capture_output=True, text=True
        )
...
def process_daily_data(session, target_date: date):
    ...
    if download_file(k_url, k_lzh):
        ...
        if k_txt:
            print("🏁 競走成績をDBへ保存中...")
            k_races, k_raw, k_results, k_payouts = parse_race_result_file(
                k_txt, -1, -1, target_date
            )
            upsert_race_results(session, k_races, [], k_results, k_payouts)
            print(
                f"✅ 競走成績: レース数={len(k_races)}, 結果数={len(k_results)}, 払戻数={len(k_payouts)}"
            )
            os.remove(k_txt)  # お掃除
        os.remove(k_lzh)
