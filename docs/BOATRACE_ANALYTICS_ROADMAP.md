...
- Phase 6 MVPは`target_win`を目的変数にしたLightGBM二値分類baselineとする
- 入力はPhase 5で生成したParquet datasetとschema JSONにする
- `pre_race_no_odds`を最初の主入力にし、オッズ込み/展示込みviewは比較対象として後続に回す
- 学習、時系列split、未来情報混入防止、レース内確率正規化、評価指標、MLflow記録、model artifact保存を完了条件にした
- `Log Loss`、`Brier Score`、`Race hit rate`、`Probability sum error`をMVP評価指標にした
- `apps/api/app/ml/`、`apps/api/tests/ml/`、`scripts/phase6_train_model.py`を実装候補の配置にした

主な未着手タスク:

- `lightgbm`、`scikit-learn`などの依存追加
- Phase 5 Parquet loaderとschema検証
- 時系列split、前処理、LightGBM学習、評価、MLflow logging
- pytest、README、構成台帳、Phase 6 docの実装結果反映
