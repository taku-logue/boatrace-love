...
- 作業ブランチ`codex/phase3-race-database`でPhase 3実装が進んでいる
- `race_id`生成処理を追加済み
...
- 2026-05-28から2026-05-30のB/Kファイルを対象に、ダウンロード、解凍、パース、DB投入するスクリプトを追加済み
- DB上では`races` 492件、`race_entries` 2,952件、`race_results` 2,832件、`payouts` 2,351件を確認済み
- `races`、`race_entries`、`race_results`、`payouts`の主要一意キー重複は0件
- 番組表と結果の両方がある`race_id`は472件

未完了または要修正:

- Phase 3 migrationの`down_revision`がPhase 2最新revisionではなく初期migrationを指している
- Phase 3 migrationにPhase 2成果物を落とす変更が混入しているため要修正
- Phase 3の`download_files`、`raw_files`、`ingestion_runs`記録が未実装
- Phase 3対象ファイルのSHA-256、文字コードメタデータ保存が未実装
- `race_card_raw`、`race_result_raw`は0件でRaw行追跡が未実装
- パイプライン対象期間がコード内固定で、`--from-date`、`--to-date`、`--venue-code`、`--dry-run`が未実装
- Prefect Flow化が未実装
- Phase 3用pytestが未実装
- API品質コマンドは通過済み。ただし、Phase 3専用fixture/pytestは未実装
- READMEと構成台帳へのPhase 3反映は一部完了。正式実行手順は未完了
