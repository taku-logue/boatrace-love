...
Phase 8 MVPまで完了済み。現在はPhase 7予測APIをNext.js Web画面から利用し、指定`raceId`の6艇1着確率ランキングを確認できる。次の中心はPhase 9のバックテスト・期待値分析で、予測確率とオッズを組み合わせた検証導線を作ること。
...
| `data/processed/reports/` | Phase 6評価レポート、importance、予測Parquet出力先 | `.gitkeep`のみ。生成物はGit管理しない |

## 4.2 Phase 9以降の作業者向け読み順

Phase 9以降を担当するAIエージェントは、次の順番で読むと取り違えにくい。

1. `docs/PROJECT_STRUCTURE_AND_FILE_INVENTORY.md`
2. `docs/PHASE8_WEB_FRONTEND.md`
3. `docs/PHASE7_PREDICTION_API.md`
4. `docs/PHASE6_MODEL_TRAINING.md`
5. `docs/PHASE5_FEATURE_ENGINEERING.md`
6. `docs/BOATRACE_ANALYTICS_ROADMAP.md`のPhase 8以降
7. `apps/web/src/app/page.tsx`
8. `apps/web/tests/dashboard.spec.ts`
9. `apps/api/app/routers/`
10. `apps/api/app/prediction/`
11. `apps/api/app/ml/`
12. `apps/api/app/features/`
13. `apps/api/app/models/`

Phase 9以降で学習済みmodel/APIを使う場合の基本方針:
