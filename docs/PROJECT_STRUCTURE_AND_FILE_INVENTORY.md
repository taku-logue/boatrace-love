...
作成日: 2026-05-30
更新日: 2026-06-07
対象ブランチ: `phase6-model-training`（main統合前）
...
Phase 6 MVPまで完了済み。次の中心はPhase 7の予測APIで、学習済みmodel bundle、前処理設定、評価レポートは`data/processed/models/`と`data/processed/reports/`へ生成する。
...
| `docker-compose.yml` | PostgreSQL、API、Web、Prefect、MLflowのローカル統合環境 | 開発者、CI候補 | Phase 6 CLIは`/data`と`/scripts`のmount、`mlflow:5000`を使う。MLflow台帳/artifactは`mlruns/`へ永続化 |
...
## 4.2 Phase 7以降の作業者向け読み順

Phase 7以降を担当するAIエージェントは、次の順番で読むと取り違えにくい。

1. `docs/PROJECT_STRUCTURE_AND_FILE_INVENTORY.md`
2. `docs/PHASE6_MODEL_TRAINING.md`
3. `docs/BOATRACE_ANALYTICS_ROADMAP.md`のPhase 7以降
4. `apps/api/app/ml/`
5. `scripts/phase6_train_model.py`
6. `docs/PHASE5_FEATURE_ENGINEERING.md`
7. `apps/api/app/features/`
8. `apps/api/app/models/`

Phase 7で学習済みmodelを使う場合の基本方針:
