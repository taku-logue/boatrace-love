...
| `docs/PHASE6_MODEL_TRAINING.md` | 機械学習モデル、評価、MLflow記録の計画 | Phase 6実装前の最初の参照先 |
...
機械学習モデル作成は設計完了、実装未着手。現時点では`docs/PHASE6_MODEL_TRAINING.md`に、LightGBM baseline、時系列split、評価指標、MLflow記録、model artifact保存、pytest、完了条件、優先順位を定義している。
...
Phase 6 MVPは、Phase 5 Parquet datasetを入力にして`target_win`のLightGBM二値分類baselineを学習し、同一`race_id`内で確率合計が1になるよう正規化し、Log Loss、Brier Score、Race hit rate、Probability sum errorを評価し、MLflowへparams/metrics/artifactsを記録するところまでを完了条件にする。
