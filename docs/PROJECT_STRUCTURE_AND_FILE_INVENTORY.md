...
現在の中心はPhase 4で、`apps/api/app/ingestion/live/`、`apps/api/app/ingestion/html_fetcher.py`、`apps/api/app/models/pre_race_info.py`、`apps/api/app/models/odds.py`、`scripts/phase4_*`が主作業範囲である。
...
## 4.2 Phase 5以降の作業者向け読み順

Phase 5以降を担当するAIエージェントは、次の順番で読むと取り違えにくい。

1. `docs/PROJECT_STRUCTURE_AND_FILE_INVENTORY.md`
2. `docs/BOATRACE_ANALYTICS_ROADMAP.md`のPhase 5以降
3. `docs/PHASE3_RACE_RESULTS_AND_CARDS_INGESTION.md`
4. `docs/PHASE4_REALTIME_DATA_INGESTION.md`
5. `apps/api/app/models/`
6. `apps/api/app/ingestion/race_quality.py`
7. `scripts/phase3_run_all_pipeline.py`と`scripts/phase4_run_live_pipeline.py`

Phase 5で特徴量生成を始める場合の基本方針:

- まず既存DBテーブルから読み出す。Rawファイルを直接特徴量生成の主入力にしない
...
- 新しい特徴量テーブルや学習用Parquetを作る場合は、先にPhase 5設計ドキュメントを`docs/`へ作る
...
当日リアルタイムデータ取得のMVP導線を実装中。現時点では、当日開催場、出走表、直前情報、気象、単勝オッズの取得、Raw保存、DB保存、品質チェック、Prefect dry-runまで確認済み。
...
- `scripts/phase4_check_quality.py`を現行schema向けに更新し、出走表、直前情報、気象、単勝オッズ、取得失敗statusを検査できるようにした
