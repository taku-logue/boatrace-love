...
- `apps/api/app/models/__init__.py`へPhase 3モデルをexportする
- Phase 3 migrationの`down_revision`と不要drop混入を修正する
- `scripts/phase3_run_all_pipeline.py`を`download_files`、`raw_files`、`ingestion_runs`対応に寄せる
- Phase 3の正式CLI引数を整備する
- Phase 3のfixtureテストを追加する
- `scripts/`を移動する場合は、Prefect Flow化と同時に行う
