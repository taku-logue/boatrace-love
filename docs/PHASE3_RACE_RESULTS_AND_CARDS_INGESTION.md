...
Phase 3のP0/P1基盤は大きく改善済み。ただし、Phase 3完了判定には以下が残る。
...
| ID | タスク | 状態 | 完了条件 |
|---|---|---|---|
| P3-301 | 競走成績レイアウトを定義する | 一部完了 | 結果行は抽出できる。年代差分、払戻全券種、例外行の検証を増やす |
| P3-302 | 番組表レイアウトを定義する | 一部完了 | 出走艇行は抽出できる。レース名、締切、距離、全場/全日検証を増やす |
...
| P3-309 | 欠場、失格、F、L、転覆などの表現を標準化する | 一部完了 | 正規化関数とfixtureは拡充済み。実データ由来の例外パターンをさらに増やす |
...
| Docker内で再現できる | 一部完了 | Compose設定とコマンドは整備済み。長期間再実行検証が不足 |
